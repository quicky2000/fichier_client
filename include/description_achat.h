...
{
  return "ClientId INTEGER, Date TEXT, MarqueId INTEGER, TypeId INTEGER, Reference TEXT, PrixFranc REAL, PrixEuro REAL, Garantie INTEGER";
}

//------------------------------------------------------------------------------
const std::string description<achat>::getTableFields(void)
{
  return "ClientId, Date, MarqueId, TypeId, Reference, PrixFranc, PrixEuro, Garantie";
}

//------------------------------------------------------------------------------
const std::string description<achat>::getUpdateFields(void)
{
  return "ClientId = $client_id, Date = $date, MarqueId = $marque_id , TypeId = $type_id , Reference = $reference, PrixFranc = $prix_franc , PrixEuro = $prix_euro, Garantie = $garantie";
}
...
const std::string description<achat>::getFieldValues(const achat & p_achat)
{
  ...
  l_result << p_achat.get_client_id() << ", \"" << p_achat.get_date() << "\", " << p_achat.get_marque_id() <<  ", " << p_achat.get_type_id() << ", \"" << p_achat.get_reference() << "\" ," << p_achat.get_prix_franc() << ", " << p_achat.get_prix_euro() <<  ", " << p_achat.get_garantie();
  return l_result.str();
}
...
achat* description<achat>::getItemFromRow(sqlite3_stmt* p_stmt)
{
  return new achat(sqlite3_column_int(p_stmt,0),//Achat Id
		   ...
		   sqlite3_column_int(p_stmt,3),//MarqueId
		   sqlite3_column_int(p_stmt,4),//TypeId
		   (const char*)sqlite3_column_text(p_stmt,5),//Reference
		   sqlite3_column_double(p_stmt,6),//PrixFranc
		   sqlite3_column_double(p_stmt,7),//PrixEuro
		   sqlite3_column_int(p_stmt,8)//Garantie
		   );
}
